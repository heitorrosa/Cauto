...
float cpsWithBursts(clickerConfig *clicker, RandomState *state) {
    ...
    if (state->remainingTicks > 0) {
        ...
        float adjustedDropChance = clicker->dropChance * timeMultiplier;
        float adjustedSpikeChance = clicker->spikeChance * (2.0f - timeMultiplier * 0.5f);

        if (baseChance < adjustedDropChance) {
            // Start drop sequence with variable intensity
            int dropIntensity = (int)randomization(1.0f, (float)clicker->dropCPS + 1.0f, state);
            state->cpsModifier = -dropIntensity;
            state->remainingTicks = (int)randomization(2.0f, 6.0f, state);
            finalCPS += state->cpsModifier;
        } else if (baseChance < adjustedDropChance + adjustedSpikeChance) {
            // Start spike sequence
            int spikeIntensity = (int)randomization(1.0f, (float)clicker->spikeCPS + 1.0f, state);
            state->cpsModifier = spikeIntensity;
            state->remainingTicks = (int)randomization(2.0f, 4.0f, state);
            finalCPS += state->cpsModifier;
        } else {
